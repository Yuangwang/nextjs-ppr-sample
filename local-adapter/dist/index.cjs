...
var require_dist2 = __commonJS({
  "../common/dist/index.js"(exports) {
    ...
    exports.updateOrCreateGitignore = exports.getBuildOptions = exports.runBuild = exports.DEFAULT_COMMAND = exports.Availability = exports.SupportedFrameworks = void 0;
    var child_process_1 = require("child_process");
    var fs = __importStar(require("node:fs"));
    var path = __importStar(require("node:path"));
    exports.SupportedFrameworks = ["nextjs", "angular"];
    var Availability;
    (function(Availability2) {
      Availability2["Runtime"] = "RUNTIME";
      Availability2["Build"] = "BUILD";
    ...
    function updateOrCreateGitignore2(dirPath, entries) {
...
${entry}`;
    ...
    exports.updateOrCreateGitignore = updateOrCreateGitignore2;
  }
});
...
var import_common = __toESM(require_dist2(), 1);
...
// dist/utils.js
var { copy, exists, writeFile, readJson, readdir, readFileSync, existsSync, ensureDir } = import_fs_extra.default;
function populateOutputBundleOptions(rootDir, appDir, nextBuildDirectory) {
  const outputBundleDir = (0, import_path.join)(rootDir, ".apphosting");
  const standaloneDirectory = (0, import_path.join)(nextBuildDirectory, "standalone");
  const standaloneAppPath = (0, import_path.join)(standaloneDirectory, process.env.MONOREPO_COMMAND ? (0, import_path.relative)(rootDir, appDir) : "");
  return {
    bundleYamlPath: (0, import_path.join)(outputBundleDir, "bundle.yaml"),
    outputDirectoryBasePath: outputBundleDir,
    outputDirectoryAppPath: standaloneAppPath,
    serverFilePath: (0, import_path.join)(standaloneAppPath, "server.js"),
    outputPublicDirectoryPath: (0, import_path.join)(standaloneAppPath, "public"),
    outputStaticDirectoryPath: (0, import_path.join)(standaloneAppPath, ".next", "static")
  };
}
function loadRouteManifest(distDir) {
  const manifestPath = (0, import_path.join)(distDir, ROUTES_MANIFEST);
  const json = readFileSync(manifestPath, "utf-8");
  return JSON.parse(json);
}
async function writeRouteManifest(distDir, customManifest) {
  const manifestPath = (0, import_path.join)(distDir, ROUTES_MANIFEST);
...
async function generateBundleYaml(opts, cwd, nextVersion, adapterMetadata) {
  await ensureDir(opts.outputDirectoryBasePath);
  const path = (0, import_path.normalize)((0, import_path.relative)(cwd, (0, import_path.join)(opts.outputDirectoryAppPath)));
  const outputBundle = {
    version: "v1",
    runConfig: {
      runCommand: `node ${(0, import_path.join)(path, "adapter", "dist", "bin", "serve.js")} ${path}`
    },
    metadata: {
      ...adapterMetadata,
      framework: "nextjs",
  ...
  if (!process.env.MONOREPO_COMMAND) {
    outputBundle.outputFiles = {
      serverApp: {
        include: [(0, import_path.normalize)((0, import_path.relative)(cwd, opts.outputDirectoryAppPath))]
      }
    };
  }
  await writeFile(opts.bundleYamlPath, (0, import_yaml.stringify)(outputBundle));
  return;
}

// dist/overrides.js
async function addRouteOverrides(distDir, middlewareMatchers) {
...
var adapter = {
  ...
  async modifyConfig(config, context) {
    ...
    if (context.phase === PHASE_PRODUCTION_BUILD) {
      return {
        ...
        images: {
          ...config.images || {},
          ...config.images?.unoptimized === void 0 && config.images?.loader === void 0 ? { unoptimized: true } : {}
        },
        headers: async () => {
  ...
  async onBuildComplete(context) {
    ...
    const outputBundleOptions = populateOutputBundleOptions(context.repoRoot, context.projectDir, nextBuildDirectory);
    await (0, import_fs_extra2.writeFile)(`${outputBundleOptions.outputDirectoryBasePath}/output.json`, JSON.stringify(context));
    const adapterMetadata = getAdapterMetadata();
