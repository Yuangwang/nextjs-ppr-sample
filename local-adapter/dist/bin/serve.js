...
import { ProcessingMode_BodySendMode, ProcessingMode_HeaderSendMode } from "../protos/envoy/extensions/filters/http/ext_proc/v3/processing_mode_pb.js";
// TODO bundle this rather than write/read from file
const buildContext = JSON.parse(readFileSync(join(process.cwd(), '.apphosting', 'output.json')).toString());
// --- FIX: Initialize missing manifest sections to prevent crashes ---
buildContext.routes = buildContext.routes || { dynamicRoutes: [] };
buildContext.outputs = buildContext.outputs || {};
buildContext.outputs.appPages = buildContext.outputs.appPages || [];
buildContext.outputs.prerenders = buildContext.outputs.prerenders || [];
// -------------------------------------------------------------------
...
// FIX: Force 0.0.0.0 to ensure Docker networking works reliably
...
//process.env.MINIMAL_MODE = "1";
// Allow the keep-alive timeout to be configured.
let keepAliveTimeout = parseInt(process.env.KEEP_ALIVE_TIMEOUT, 10);
// Load the Next.js configuration from the standalone directory.
// TODO fix the types and jazz
const conf = { ...buildContext.config };
conf.compress = false;
// --- FIX: Next.js 16 Canary config polyfills ---
if (!conf.amp)
    conf.amp = { canonicalBase: '' };
if (!conf.publicRuntimeConfig)
    conf.publicRuntimeConfig = {};
if (!conf.serverRuntimeConfig)
    conf.serverRuntimeConfig = {};
// ----------------------------------------------
...
const middlewareMatchers = buildContext.outputs?.middleware?.config?.matchers?.map((it) => new RegExp(it.sourceRegex)) || [];
const isPPRPath = (path) => {
    const dynamicPathMatch = buildContext.routes.dynamicRoutes.find((it) => path.match(new RegExp(it.sourceRegex)))?.source;
    const matchingPrerender = buildContext.outputs.prerenders.find((it) => it.pathname === dynamicPathMatch || it.pathname === path);
    return matchingPrerender?.config?.renderingMode === "PARTIALLY_STATIC";
};
const getAppPage = (path) => {
    const dynamicPathMatch = buildContext.routes.dynamicRoutes.find((it) => path.match(new RegExp(it.sourceRegex)))?.source;
    return buildContext.outputs.appPages.find((it) => it.id === (dynamicPathMatch || path));
};
const getPrerender = (path) => {
    const dynamicPathMatch = buildContext.routes.dynamicRoutes.find((it) => path.match(new RegExp(it.sourceRegex)))?.source;
    return buildContext.outputs.prerenders.find((it) => it.id === (dynamicPathMatch || path));
};
const getMatchedDestination = (path) => {
    for (const dynamicRoute of buildContext.routes.dynamicRoutes) {
        const match = path.match(new RegExp(dynamicRoute.sourceRegex));
        if (match) {
            return Object.entries(match.groups || {}).reduce((acc, [key, value]) => { return acc.replace(`\$${key}`, value); }, dynamicRoute.destination);
        }
    }
};
 ...
 */
async function injectAppHostingHeaders(req, res) {
    if (req.method !== 'GET' && req.method !== 'HEAD')
        return;
    if (!res.getHeaderNames().includes('x-nextjs-postponed'))
        return;
    ...
    const metadata = req[NEXT_REQUEST_META];
    // TODO use a stable API to get the resumption token
...
async function requestHandle(req, res) {
    const path = parse(req.url || '/', true).path || "/";
    const isPPR = ['GET', 'HEAD'].includes(req.method || 'GET') && isPPRPath(path);
    const prerender = getPrerender(path);
    const postponed = prerender?.fallback.postponedState;
    // TODO this is a bug with the adapter spec, not seeing postponeData for "/"
    if (isPPR && !postponed) {
        console.error(`Unable to find postponed state for ${path} appending to response as x-fah-postponed`);
        /**
        ...
        res.write = function () {
            resolveHeaders || (resolveHeaders = injectAppHostingHeaders(req, res));
            resolveHeaders.then(() => {
 ...
 */
const grpcServer = fastify({ http2: true });
await grpcServer.register(fastifyConnectPlugin, {
    routes: (router) => router.service(ExternalProcessor, {
        ...
        process: async function* processCallouts(callouts) {
            let requestHeaders = [];
            let resolveResumeBuffer;
            let path = undefined;
            let method = undefined;
            let postponed = undefined;
            ...
            const getRequestHeader = (key) => {
                ...
                if (header)
                    return header.value || new TextDecoder().decode(header.rawValue);
                return undefined;
            };
            for await (const callout of callouts) {
                let modeOverride = {}; // TODO fix types
                if (path)
                    console.log(method, callout.request.case, path);
                switch (callout.request.case) {
                    case "requestHeaders": {
                        ...
                        path = parse(getRequestHeader(":path") || "/", true).path;
                        method = getRequestHeader(":method") || "GET";
                        console.log(method, "requestHeaders", path);
                        const hasMiddleware = middlewareMatchers.some((it) => path?.match(it));
                        const usesPPR = ['GET', 'HEAD'].includes(method) && isPPRPath(path);
                        const hasRequestBody = !callout.request.value.endOfStream;
                        // TODO only response on PPR
                        if (hasMiddleware && hasRequestBody) {
                            modeOverride.requestBodyMode = ProcessingMode_BodySendMode.BUFFERED;
                        }
                        if (usesPPR) {
                            modeOverride.responseHeaderMode = ProcessingMode_HeaderSendMode.SEND;
                            modeOverride.responseBodyMode = ProcessingMode_BodySendMode.FULL_DUPLEX_STREAMED;
                            modeOverride.responseTrailerMode = ProcessingMode_HeaderSendMode.SEND;
                        }
                        ;
                        if (!hasMiddleware)
                            break;
                    ...
                    case "requestBody": {
                        ...
                        if (!middlewareMatchers.some((it) => path?.match(it)))
                            break;
                        ...
                        try {
                            // --- FIX: Robust middleware handler resolution ---
                            // Different builds/versions structure the export differently.
                            // We check multiple locations to find the actual function.
                            ...
                            if (typeof handler !== 'function') {
                                throw new Error(`Middleware export structure not recognized. Keys: ${Object.keys(mod.default || mod)}`);
                            }
                            const result = await handler({ request: middlewareRequest });
                            // -------------------------------------------------
                        ...
                        if (middlewareResponse.headers.has("x-middleware-next"))
                            break;
                    ...
                    case "responseHeaders": {
                        ...
                        const isPPR = ['GET', 'HEAD'].includes(method) && isPPRPath(path);
                        if (!isPPR)
                            break;
                        const postponedHeaderValue = callout.request.value.headers?.headers.find((it) => it.key === "x-fah-postponed")?.rawValue;
                        if (postponedHeaderValue) {
                            const tokenString = new TextDecoder().decode(postponedHeaderValue);
                            postponed = Buffer.from(tokenString, "base64url").toString();
                        }
                        ...
                        resolveResumeBuffer = new Promise(async (resolve) => {
                            const socket = new Socket();
                            const resumeRequest = new IncomingMessage(socket);
                            const prerender = getPrerender(path);
                            postponed || (postponed = prerender?.fallback.postponedState);
                            ...
                            resumeResponse.write = (data) => {
                                ...
                                if (!result)
                                    intermediaryStream.on("drain", () => resumeResponse.emit("drain"));
                                return result;
                            };
                            resumeResponse.end = (data) => {
                                if (data)
                                    intermediaryStream.push(data);
                                intermediaryStream.end();
                                return resumeResponse;
                            };
                            const parsedUrl = parse(path, true);
                            const appRouteFunction = await import(appPage.filePath);
                            appRouteFunction.default.handler(resumeRequest, resumeResponse, parsedUrl);
                    ...
                    case "responseBody": {
                        ...
                        // case again.
                        if (!callout.request.value.endOfStream)
                            continue;
                            ...
                            yield {
                                response: {
                                    ...
                                    value: {
                                        response: {
                                            status: CommonResponse_ResponseStatus.CONTINUE_AND_REPLACE,
                                            bodyMutation: { mutation: { case: 'streamedResponse', value: { body: Uint8Array.from(body), endOfStream: false } } },
                                            end_stream: false,
                                        },
                                    },
                                },
