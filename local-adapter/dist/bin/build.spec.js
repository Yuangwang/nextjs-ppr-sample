...
describe("build commands", () => {
    let tmpDir;
    let outputBundleOptions;
    let defaultNextVersion;
    let adapterMetadata;
    beforeEach(() => {
...
`,
    ...
    it("copies files into correct location in a monorepo setup", async () => {
        ...
        generateTestFiles(tmpDir, files);
        await generateBuildOutput(tmpDir, "apps/next-app", {
            bundleYamlPath: path.join(tmpDir, ".apphosting", "bundle.yaml"),
            outputDirectoryBasePath: path.join(tmpDir, ".apphosting"),
            outputDirectoryAppPath: path.join(tmpDir, ".next", "standalone", "apps", "next-app"),
            outputPublicDirectoryPath: path.join(tmpDir, ".next", "standalone", "apps", "next-app", "public"),
            outputStaticDirectoryPath: path.join(tmpDir, ".next", "standalone", "apps", "next-app", ".next", "static"),
    ...
    it("test failed validateOutputDirectory", async () => {
        ...
        generateTestFiles(tmpDir, files);
        await generateBuildOutput(tmpDir, tmpDir, outputBundleOptions, path.join(tmpDir, ".next"));
        assert.rejects(async () => await validateOutputDirectory(outputBundleOptions, path.join(tmpDir, ".next")));
    });
    ...
    it("test populate output bundle options", async () => {
        ...
        assert.deepEqual(populateOutputBundleOptions("test", "test", "test/.next"), expectedOutputBundleOptions);
    });
    afterEach(() => {
...
function generateTestFiles(baseDir, filesToGenerate) {
...
function validateTestFiles(baseDir, expectedFiles) {
...
function validatePartialYamlContents(baseDir, yamlFileName, expectedPartialYaml) {
